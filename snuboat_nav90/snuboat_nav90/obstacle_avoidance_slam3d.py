...
class Obstacle_Avoidance(Node):
    ...
    def __init__(self):
        ...
        # self.ref_safe_radius = 2.0

        # self.safe_radius = 1.7
        # self.ref_safe_radius = 1.7

        self.goal_tol = 1.3
        self.wp_stay_time = 30
        self.inf_length = self.B * 1.1
        self.ref_inf_length = self.B * 1.1
        ...
        self.current_pos_sub = self.create_subscription(
            PoseStamped,"/current_pose",self.current_pose_callback, 1
        )
    ...
    def shape_detect_callback(self, msg):
        if msg.data[0] == 1: 
            ...
            if msg.data[1] == -1:
                ...
                # self.odom_wp_x_set[self.dock_wp_idx] = self.dock_wp_x_set[0]
                # self.odom_wp_x_set[self.dock_wp_idx] = self.dock_wp_y_set[0]
            elif msg.data[1] == 0:
                self.get_logger().info("Middle")
                # self.odom_wp_x_set[self.dock_wp_idx] = self.dock_wp_x_set[1]
                # self.odom_wp_x_set[self.dock_wp_idx] = self.dock_wp_y_set[1]
            elif msg.data[1] == 1:
                self.get_logger().info("Right")
                # self.odom_wp_x_set[self.dock_wp_idx] = self.dock_wp_x_set[2]
                # self.odom_wp_x_set[self.dock_wp_idx] = self.dock_wp_y_set[2]
