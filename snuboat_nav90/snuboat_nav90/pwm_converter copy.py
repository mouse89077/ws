...
class PWMConverter(Node):
    ...
    def pub_pwm(self): 
        if self.docking_state == False:
            ...
            if self.detect_state == False: 
                self.cal_self_rotate()
            # images all detected and waypoint changed --> operate as usual
            else: 
                if self.wp_check == False:
                    if len(self.safe_heading) != 0:
                        if abs(self.err_heading[-1]) < 90.0:
